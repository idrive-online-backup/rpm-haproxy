...
run-docker: build-docker
	...
	docker run --volume $(HOME)/RPMS:/RPMS --env USE_LUA=$(USE_LUA) --env USE_PROMETHEUS=$(USE_PROMETHEUS) --env EXTRA_CFLAGS="$(EXTRA_CFLAGS)" --rm haproxy-rpm-builder7:${VERSION}-${RELEASE}
	docker run --volume $(HOME)/RPMS:/RPMS --env USE_LUA=$(USE_LUA) --env USE_PROMETHEUS=$(USE_PROMETHEUS) --env EXTRA_CFLAGS="$(EXTRA_CFLAGS)" --rm haproxy-rpm-builder8:${VERSION}-${RELEASE}
